function SubmithandoverTC(){
   const handoverTCForm=getID("handoverTankChipForm");
     handoverTCForm.addEventListener('submit', function(e) {
     e.preventDefault();
     e.stopImmediatePropagation();
     let btnSubmit=getID("submitTCBtn").disabled = true;
     const formData = new FormData(this);
     formData.append("handoverTCSubmit",1);
     ...
     .then((data) => {
       if(data.status == 1){
         ...
         getID("submitTCBtn").disabled = false;
         myToast(data.status,data.message);
       }
     });
   })
...
function SubmitreturnTC(){
   const returnTCForm=getID("returnTankChipForm");
     returnTCForm.addEventListener('submit', function(e) {
     e.preventDefault();
     e.stopImmediatePropagation();
     let submitBtn=getID("submitReturnTC");
     submitBtn.disabled=true;
     const formData = new FormData(this);
     formData.append("returnTCSubmit",1);
       fetch('../../ajax/ajax_employee.php',{
       method: 'POST',
       body: formData,
     })
     ...
     .then((data) => {
       if(data.status == 1){
         // getID('returnTCForm').reset();
           myToast(data.status,data.message);
          setTimeout(function(){ location.reload(); }, 3000);
       }else{
         getID("submitReturnTC").disabled=false;
         myToast(data.status,data.message);
       }
     });
   })
}
queryAll(".printTCHandover").forEach((row,index)=>{
  row.addEventListener("click",()=>{
    let tcHandovedId=getAttr(row,"data-print");
    let cid=getID("cid").value;
    formdata=new FormData();
    formdata.append("getPrintHandover","tank_chip_handover");
    // formdata.append("getTCPrintHandover",1);
 ...
 if(data.signature_staff != ''){
  getID("handoverSignStaff").innerHTML=`<img src="../../img/signature_uploads/${data.signature_staff}" style="width:95%"><p>${data.signed_by}</p>`;
 }
...
queryAll(".printTCReturn").forEach((row,index)=>{
  row.addEventListener("click",()=>{
    let tcReturnId=getAttr(row,"data-print");
    let cid=getID("cid").value;
    formdata=new FormData();
    formdata.append("getPrintReturn","tank_chip_return");
    // formdata.append("getTCPrintReturn",1);
...
saveHandoverPrint.addEventListener("click",()=>{
 if(texthandover.value.trim() != '' && textareaHandover.value.trim() != ''){
  ...
  formdata.append("savePrint","tank_chip_handover");
...
saveReturnPrint.addEventListener("click",()=>{
 if(textReturn.value.trim() != '' && textareaReturn.value.trim() != ''){
  ...
  let formdata=new FormData();
  formdata.append("savePrint","tank_chip_return");
