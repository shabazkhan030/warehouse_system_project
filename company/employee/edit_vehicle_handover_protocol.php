...
$hm=$_GET["hm"];
$vlid=$_GET["vlid"];
$qry=mysqli_query($dbcon,"SELECT * FROM vehicle_log_protocol WHERE hallmark='$hm' AND id='$vlid'");
  ...
  <div class="d-flex">
    ...
    <div class="oben1 obenr">
      <div><small class="d-inline-block text-light p-10 bg-lightblue">Fahrzeugprotokol</small>
				<small class="d-inline-block text-light p-10 bg-red">Übergabeprotokoll</small></div>
   ...
   <form id="editVehicleLogHandoverForm">
	<div class="my-100" style="display:flex">
		...
		<div class="w-50">		
			<!--<div class="ml-50">
				...
				<small class="d-inline-block text-light p-5 bg-grey">Fahrzeugübergabeprotokoll</small>
	    ...
	    <div class="d-block mr-20 ml-10 mb-20 mt-20">
		 	    <div class="d-flex">
			 	    	<div class="d-block">
 ...
 ?>
		...
		<div class="w-50">
			<!--<div class="d-inline-block p-5 ml-20 bg-orange text-light">
				text vehicle handover Protocol
			</div>-->
...
<?php 
 ...
 ?>	
			  <div class="d-block">				
			  	    <label class="font-weight-bold" style="margin-top:50px;">Unterschrift Fahrzeugnutzer</label>
			  	    <br>	
...
if($row5 > 0){
 ...
 ?>
		...
		<!--<input type="submit" name="" value="Save" class="btn bg-grey" style="border:1px solid grey;"> -->
			</div>
			<div class="mt-50 ml-20">
				  <div id="subtext">
				    <!-- <div id="vehicle_list_subtext"></div> -->
<?php 
...
?>
	...
	<div class="mt-100 text-center mb-50">
			<?php
      if(empty($arr["sign_fleet_manager"]) || empty($arr["sign_vehicle_manager"]) || $arr["data"]=='0000-00-00 00:00:00'){
 	?>
       <input type="submit" class="btn-sm bg-grey cur-pointer" value="Alles Speichern" onclick="vehicleLogHandoverEditSave();"> 
   <?php          
   }else{
	?>
...
<?php include "../print/vehicle_log.php";  ?>
	...
	<script>
       ...
       function SaveSubtext(){
       	var subtxt=[];
         queryAll(".subtext").forEach((row,index)=>{
         	subtxt.push(row.value);
         });
         let formdata=new FormData();
         formdata.append('saveSubtxt',1);
         formdata.append('cid',)
         formdata.append('subtxt',JSON.stringify(subtxt));
         fetch("../../ajax/ajax_employee",{
         	method:"POST",
         	body:formdata
         }).then((res)=>{
         	return res.json();
     ...
     function vehicleLogHandoverEditSave(){
       	var formID=getID("editVehicleLogHandoverForm");
