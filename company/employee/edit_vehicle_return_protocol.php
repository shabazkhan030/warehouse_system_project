...
$qryST2=mysqli_query($dbcon,"SELECT * FROM vehicle_log_protocol WHERE created_id='$created_id' AND stand='1'");
$arr=mysqli_fetch_assoc($qry);
$arrST2=mysqli_fetch_assoc($qryST2);
$vlid=$arr["id"];
$stand=$arr["stand"];
?>
<!DOCTYPE html>
  ...
  <div class="d-flex">
    ...
    <div class="oben1 obenr">
      <div><small class="d-inline-block text-light p-10 bg-lightblue">Fahrzeugprotokol</small>
				<small class="d-inline-block text-light p-10 bg-red">Rückgabeprotokoll</small></div>
   ...
   <form id="editVehicleLogReturnForm">
		...
		<div class="w-50">		
			...
			<div class="ml-10 mt-20 d-flex">
               <div class="d-block mt-20 ml-10">
               	   ...
               	   <input type="hidden" name="vlid" value="<?php echo $vlid; ?>">
               	   <input type="hidden" name="crid" value="<?php echo $created_id; ?>">
               	   <label>Kennzeichen</label><br>

               	   <select name="hallmark">
               	   <?php 	
	                    $qry2=mysqli_query($dbcon,"SELECT vl.hallmark AS hallname,vl.id,vlp.hallmark FROM vehicle_log_protocol vlp INNER JOIN vehicle_list vl ON vlp.hallmark=vl.id WHERE vlp.cid='$cid' AND vlp.created_id='$created_id' AND vlp.stand='$stand'");
	                     if($qry2){
               ...
               <div class="d-block mt-20 ml-10">
               	   <label>Datum Übergabe</label><br>
				       <input type="datetime-local"  value="<?php echo date('Y-m-d\TH:i:s', strtotime($arr["data"])); ?>" name="returndate" >
               </div>
               <div class="d-block mt-20 ml-10">
			...
			<div>
				...
				<label>Der Fahrzeugnutzer hat folgende Unterlagen erhalten:</label>
					<thead>
						<tr>
							<td>&nbsp;</td>
							<td>JA</td>
...
<?php
$qryCheckbox=mysqli_query($dbcon,"SELECT * FROM vehicle_log_checkbox_return WHERE vlid='$vlid'");
  		...
  		?>
  		   <div class="d-block mt-20 mr-20 w-80">
			   ...
			   <div class="d-block"><br><br>
		          <a href="javascript:void(0)" class="cancel-btn text-decoration mt-50" data-delete="<?= $arrComment['id'] ?>"aria-label="Close" onclick="confirmModalComment(this)"></a>
		          <!-- deletecomment(this); -->
		     </div>
<?php
  	}	
...
<?php 
 ...
 ?>			
            <div class="d-block  mb-20 mr-20">
               <label class="font-weight-bold">Unterschrift Fuhrparkleiter</label><br>
					<div id="signaturePurchaserWrapper" style="border:solid 2px black;width:400px;height:150px;padding:3px;">
 ...
 ?>					
            <div class="d-block">
              <label class="font-weight-bold" style="margin-top:50px;">Unterschrift Fahrzeugnutzer</label><br>	
				  <div id="signatureDirectorWrapper" style="border:solid 2px black;width:400px;height:150px;padding:3px;">  
	...
	while($arr5=mysqli_fetch_assoc($qry5)){
		?>
			 <tr>
			      ...
			      <td>
			      	<div class="d-block">
		              <a href="javascript:void(0)" class="cancel-btn text-decoration mt-50" data-delete="<?= $arr5['id'] ?>"aria-label="Close" onclick="confirmModalCheckbox2(this);"></a>
		              <!-- deleteCheckbox2(this) -->
		            </div>
			      </td>
			 </tr>
...
if($rowsvls > 0){
 ...
 ?>
          <div class="d-block mt-20">
					    ...
					    <a href="javascript:void(0)" class="cancel-btn text-decoration ml-10" data-delete="<?= $arrvls['id'] ?>"aria-label="Close" onclick="confirmModalSubtext(this)"></a>
					    <!-- deletesubtext(this); -->
					</div>
...
?>
					...
					?>
                        <div style="display:block;width:300px;border:1px;">
	      	               <div style="display:flex">
	                          ...
	                          <a href="javascript:void(0)" class="cancel-btn text-decoration mt-50 ml-20" aria-label="Close" data-delete="<?php echo $arrImg["id"] ?>" onclick="confirmModalImage(this)"></a>	
	                           <!-- deleteVehicleImage(this) -->
	                        </div>
					         </div>
					   <br>
					<?php
	...
	<?php
      if(empty($arr["sign_fleet_manager"]) || empty($arr["sign_vehicle_manager"]) || $arr["data"]=='0000-00-00 00:00:00' || empty($arrST2["sign_fleet_manager"]) || empty($arrST2["sign_vehicle_manager"]) || $arrST2["data"]=='0000-00-00 00:00:00'){
 	?>
       <input type="submit" class="btn-sm bg-grey cur-pointer" value="Alles Speichern" onclick="vehicleLogReturnEditSave();"> 
   <?php          
   }else{
	?>
	   <a href="vehicle_log.php?eid=<?php echo $_GET["eid"]; ?>" class="btn bg-red cur-pointer">Abgeschlossen</a>
	<?php
	}
	?>	  
	</div>
</form>
<div id="confirmModalSubtext" class="modal">

  <!-- Modal content -->
  <div class="modal-content">
    <!-- <span class="closeModal">&times;</span> -->
    <div class="text-center pt-50">
    	<p>Are you sure you want to delete Subtext ?</p>
    	<input type="hidden" id="dataDelete" value="">
    	<div class="d-flex" style="justify-content:center;padding-top:30px;padding-bottom:20px;">
    		 <a href="javascript:void(0)" class="btn bg-green" onclick="confirmDeleteSubtext()">Yes</a>
    		 <a href="javascript:void(0)" class="closeModal btn bg-red mr-20">Close</a>
    	</div>
    </div>
  </div>
</div>
<div id="confirmModalComment" class="modal">
  <!-- Modal content -->
  <div class="modal-content">
    <!-- <span class="closeModal">&times;</span> -->
    <div class="text-center pt-50">
    	...
    	<input type="hidden" id="dataDelete" value="">
    	<div class="d-flex" style="justify-content:center;padding-top:30px;padding-bottom:20px;">
    		 <a href="javascript:void(0)" class="btn bg-green" onclick="confirmDeleteComment()">Yes</a>
    		 <a href="javascript:void(0)" class="closeModal btn bg-red mr-20">Close</a>
    	</div>
    </div>
  </div>
</div>
<div id="confirmModalCheckbox2" class="modal">
  <!-- Modal content -->
  <div class="modal-content">
    <!-- <span class="closeModal">&times;</span> -->
    <div class="text-center pt-50">
    	...
    	<input type="hidden" id="dataDelete" value="">
    	<div class="d-flex" style="justify-content:center;padding-top:30px;padding-bottom:20px;">
    		 <a href="javascript:void(0)" class="btn bg-green" onclick="confirmDeleteCheckbox2()">Yes</a>
    		 <a href="javascript:void(0)" class="closeModal btn bg-red mr-20">Close</a>
    	</div>
    </div>
  </div>
</div>
<div id="confirmModalImage" class="modal">
  <!-- Modal content -->
  <div class="modal-content">
    <!-- <span class="closeModal">&times;</span> -->
    <div class="text-center pt-50">
    	...
    	<input type="hidden" id="dataDelete" value="">
    	<div class="d-flex" style="justify-content:center;padding-top:30px;padding-bottom:20px;">
    		 <a href="javascript:void(0)" class="btn bg-green" onclick="confirmDeleteImage()">Yes</a>
    		 <a href="javascript:void(0)" class="closeModal btn bg-red mr-20">Close</a>
    	</div>
    </div>
  </div>
	...
	<?php include "../print/vehicle_log.php";  ?>  
	...
	<script>
     ...
     function vehicleLogReturnEditSave(){
       	var formID=getID("editVehicleLogReturnForm");
      ...
      function deleteVehicleImage(el){
          let imgId=el;
          let formdata=new FormData();
          formdata.append("deletevehiclelogImage",1)
